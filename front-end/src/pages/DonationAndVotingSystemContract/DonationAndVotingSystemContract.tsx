...
const DonationAndVotingSystemContractPage = () => {
    ...
    // 最大投票次数
    const [maxVotingTimes, setMaxVotingTime] = useState(0)
    // 领取金币Gold的数量
            ...
            try {
                ...
                const __userVotesInformation = _userVotesInformation[0].map((item: number, index: number) => {
                    return {
                        ...
                        donationIdVoteOn: +_userVotesInformation[2][index]
                    }
                })
            ...
            try {
                ...
                const _donationInfo = await Promise.all(__donationsIds.map(async (id: number) => {
                    try {
                        ...
                        const _donationGetGoldReward = await DonationAndVotingSystemContract_Contract.methods.getWhetherUserCanGetAwardReward(id, timeNow()).call({from: account})
                        return {
                            id: id,
    ...
    const getInitialUserGold = async () => {
        if (DonationAndVotingSystemContract_Contract) {
            try {
                const _initialUserGold = await DonationAndVotingSystemContract_Contract.methods.getInitialUserGold().call({from: account})
    ...
    const getGold = async () => {
        ...
        if (DonationAndVotingSystemContract_Contract) {
            try {
                await DonationAndVotingSystemContract_Contract.methods.getGold().send({from: account})
                getUserInfo()
                getDonationInfo()
    ...
    // 手动领取金币奖励
    const getGoldRewardFromDonationApproval = async (id: number) => {
        if (account === '') {
            setErrorMessage('你尚未连接钱包！')
            return
        }
        if (DonationAndVotingSystemContract_Contract && GoldContract_Contract) {
            try {
                await DonationAndVotingSystemContract_Contract.methods.getGoldRewardFromDonationApproval(id).send({from: account})
                getUserInfo()
                getDonationInfo()
                        ...
                        button_action =
                            <div>
                                ...
                                <Popconfirm title={"投票将消耗" + goldConsumedByVote + " Gold。你目前拥有" + userInfo.balance + " Gold。确定继续吗？"} onConfirm={() => voteOnDonation(0, item.id)} okText="确定" cancelText="取消" placement="leftTop">
                                    <Button icon={<CheckSquareOutlined />}>反对</Button>
                                </Popconfirm>
                            </div>;
                    } else if (item.status === 2 && item.getGoldReward === true) {
                        button_action = <Button icon={<DollarCircleOutlined />} onClick={() => getGoldRewardFromDonationApproval(item.id)}>领取金币Gold奖励</Button>;
    ...
    useEffect(() => {
        if (account !== "" && donationsInfo && userInfo.donationIds) {
            try {
                    ...
                    if (item.status === 0) {
                        button_action =
                            <div>
                                ...
                                <Button icon={<CheckSquareOutlined />} onClick={() => voteOnDonation(0, item.id)}>反对</Button>
                            </div>;
                    } else if (item.status === 2 && item.getGoldReward === true) {
                        button_action =
                            <Button icon={<DollarCircleOutlined />} onClick={() => getGoldRewardFromDonationApproval(item.id)}>领取金币Gold奖励</Button>;
                    }
                    return {
                        id: item.id,
                        voteStartTime: getDate(item.voteStartTime),
                ...
                _userDonationData.forEach((item, , ) => {
            ...
            try {
                setUserVoteData(userInfo.votesInfo.map((item) => {
                    return {
                        ...
                        donationIdVotedOn: item.donationIdVotedOn,
    ...
    const DonationCenter = () => {
        return (
            <Layout className="site-layout" style={{ marginLeft: 200, minHeight: 900}}>
                {(errerMessage !== "" && open === false) && <Alert type="error" message={errorMessage} banner closable afterClose={()=>setErrorMessage("")}></Alert>}
